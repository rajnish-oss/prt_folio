...
const defaultColors = ['#5227FF', '#d9ed27', '#f95b5b'];
...
export default function LiquidEther({
  ...
  // Optimize parameters for low-end devices
  const optimizedMouseForce = isLowEnd ? mouseForce * 0.7 : isMobile ? mouseForce * 0.85 : mouseForce;
  const optimizedCursorSize = isLowEnd ? cursorSize * 0.6 : isMobile ? cursorSize * 0.75 : cursorSize;
  const optimizedIterationsViscous = isLowEnd ? Math.max(4, Math.floor(iterationsViscous * 0.4)) : isMobile ? Math.floor(iterationsViscous * 0.7) : iterationsViscous;
  const optimizedIterationsPoisson = isLowEnd ? Math.max(4, Math.floor(iterationsPoisson * 0.4)) : isMobile ? Math.floor(iterationsPoisson * 0.7) : iterationsPoisson;
  const optimizedResolution = isLowEnd ? 0.2 : isMobile ? 0.25 : resolution;
  const optimizedBFECC = !isLowEnd && BFECC; // Disable BFECC on low-end
  const optimizedAutoIntensity = isMobile ? autoIntensity * 0.8 : autoIntensity;
  ...
  useEffect(() => {
    ...
    class CommonClass {
      ...
      init(container: HTMLElement) {
        ...
        this.pixelRatio = isLowEnd ? 1 : isMobile ? Math.min(baseDPR, 1.5) : Math.min(baseDPR, 2);
        this.resize();
        this.renderer = new THREE.WebGLRenderer({ antialias: !isLowEnd, alpha: true, powerPreference: isLowEnd ? 'low-power' : 'high-performance' });
        // Always transparent
  ...
  useEffect(() => {
    ...
    if (webgl.autoDriver) {
      ...
      if (webgl.autoDriver.mouse) {
        webgl.autoDriver.mouse.autoIntensity = optimizedAutoIntensity;
